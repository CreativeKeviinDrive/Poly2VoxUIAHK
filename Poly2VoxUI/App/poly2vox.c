...
static int xsiz, ysiz, zsiz, rendermode = 0, gtexfilter = 4, savetype;
...
static int loadpoly (char *ofnam)
{
	...
	if ((fnlng >= 4) && (!strcmp(&fnam[fnlng-4],".zip"))) kzaddstack(fnam);
	else
	{
		if (fnlng >= 4)
		{
		...
		strcpy(&fnam[fnlng],".zip"); kzaddstack(fnam);
	}
...
static void showhelp (void)
{
	...
	printf(" /c   Center model extents at origin (default is to use polygon file's 0,0,0)\n");
...
void main (int argc, char **argv)
{
	...
	for(i=1;i<argc;i++)
	{
		...
		switch(j)
		{
			...
			case 'Z': j = 2; if (!argv[i][2]) { i++; j -= 2; } kzaddstack(&argv[i][j]); break;
...
// Scale data to specified resolution ---------------------------------------
	...
	if (centmode) //center model extents around origin
	{
		xpiv = ((float)xsiz)*.5;
		ypiv = ((float)ysiz)*.5;
		zpiv = ((float)zsiz)*.5;
